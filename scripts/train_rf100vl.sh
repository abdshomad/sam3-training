...
if [ "${DRY_RUN}" = true ]; then
    echo "Would run: bash scripts/run_training.sh ${TRAINING_ARGS[*]}"
    echo ""
    echo "Dry run complete. Remove --dry-run to execute training."
    exit 0
else
    echo "Executing: bash scripts/run_training.sh ${TRAINING_ARGS[*]}"
    echo ""
    bash scripts/run_training.sh "${TRAINING_ARGS[@]}"
    TRAINING_EXIT_CODE=$?

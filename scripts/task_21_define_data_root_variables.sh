...
if [ -n "${ROBOFLOW_ROOT}" ]; then
    if [ ! -d "${ROBOFLOW_ROOT}" ]; then
        echo "ERROR: Roboflow root path does not exist or is not a directory: ${ROBOFLOW_ROOT}"
        exit 1
    fi
    # Convert to absolute path
    ROBOFLOW_ROOT="$(cd "${ROBOFLOW_ROOT}" && pwd)"
...
if [ -n "${ODINW_ROOT}" ]; then
    if [ ! -d "${ODINW_ROOT}" ]; then
        echo "ERROR: ODinW root path does not exist or is not a directory: ${ODINW_ROOT}"
        exit 1
    fi
    # Convert to absolute path
    ODINW_ROOT="$(cd "${ODINW_ROOT}" && pwd)"

...
if [ "${SKIP_ENV_SETUP}" = false ]; then
    ...
    "${PROJECT_ROOT}/scripts/task_10_environment_preparation.sh"
    if [ $? -ne 0 ]; then
...
if [ "${SKIP_CONFIG_VALIDATION}" = false ]; then
    ...
    "${PROJECT_ROOT}/scripts/task_25_config_file_existence_check.sh" -c "${CONFIG_ARG}"
    if [ $? -ne 0 ]; then
...
"${PROJECT_ROOT}/scripts/task_30_script_argument_parsing.sh" "${ARG_PARSING_ARGS[@]}"
if [ $? -ne 0 ]; then
...
set +e
source "${PROJECT_ROOT}/scripts/task_40_execution_logic_construction.sh" "${EXECUTION_ARGS[@]}"
EXECUTION_EXIT_CODE=$?
set -e
...
"${PROJECT_ROOT}/scripts/task_50_post_execution_and_monitoring.sh" "${POST_EXEC_ARGS[@]}" || {
    POST_EXEC_EXIT_CODE=$?
